...
WC.Bioclim <- rast(x = "DATA/wc2.1_10m_bio_1.tif")
...
NoAm.WC.Bioclim.AEA <- terra::project(
  x = NoAm.WC.Bioclim,
  y = AEA.proj,
  filename = "DATA/NoAm_Bioclim_AEA.tif",  # write to disk to not overload memory
  overwrite = TRUE
)
...
plot(st_geometry(Bigfoot.SpaPnt.AEA[rows_with_na$row_number, ]),
     add = TRUE,
     pch = 19,
     col = "black",
     cex = 1.5)
...
ggplot() +
  ...
  geom_point(data = Bigfoot.SpaPnt.AEA, color = "red", size = 2) +
  ...
  # Add title and theme
  ggtitle(title = "Bigfoot Sightings and Annual Mean Temperature") +
  theme_minimal() +
